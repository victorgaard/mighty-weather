...
function App() {
	...
	async function getTimezone(lat, lon) {
		try {
			const response = await fetch(`${process.env.REACT_APP_URL_OW_TZ}&lat=${lat}&lon=${lon}&exclude=hourly,daily,minutely&units=metric&appid=${process.env.REACT_APP_API_KEY_OW}`)
	...
	async function getWeather(lat, lon) {
		...
		if (lat && lon) {
			...
			try {
				const response = await fetch(`${process.env.REACT_APP_URL_OW}&lat=${lat}&lon=${lon}&units=metric&appid=${process.env.REACT_APP_API_KEY_OW}`)
				...
				if (response.ok) {
					...
					const newWeather = { city: city, sky: sky, description: description, iconId: iconId, temp: temp, feelsLike: feelsLike, maxTemp: maxTemp, minTemp: minTemp, humidity: humidity };
